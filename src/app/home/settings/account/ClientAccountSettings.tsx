...
export default function ClientAccountSettings({ initialData }: Props) {
  ...
  const handleSaveChanges = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setUserData(prev => ({ ...prev, username: formData.get('username') as string, email: formData.get('email') as string }))
      } else {
  ...
  const handleCancelDeletion = async () => {
    try {
      const result = await fetch('/api/v1/settings/account', { method: 'DELETE' }).then(res => res.json())
      if (result.success) {
        toast.success(result.message)
  ...
  return (
    <div>
      ...
      <Card className="mb-6 bg-neutral-800 text-white border-neutral-700">
        <CardHeader>
          <CardTitle className="text-red-500 flex items-center">
            <AlertTriangle className="text-red-500 mr-2 h-6 w-6" /> Gevaarlijke zone
          </CardTitle>
          <CardDescription className="text-neutral-400">
            ...
            <div className="bg-yellow-900/30 border border-yellow-700 rounded-md p-4">
              ...
              <p className="text-sm mb-3">
                ...
                <span className="font-semibold">
                  {new Date(userData.scheduledDeletion).toLocaleDateString('nl-NL', {
                    day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit'
                  })}
                </span>
              </p>
              <Button1 text="Verwijdering annuleren" onClick={handleCancelDeletion} />
            </div>
          ) : (
            <>
              <h3 className="font-medium">Account verwijderen</h3>
              <p className="text-sm text-neutral-400">
                Verwijder permanent je account en alle bijbehorende gegevens. Dit kan niet ongedaan gemaakt worden.
              </p>
              <Button1 text="Account verwijderen" onClick={() => setDeleteDialogOpen(true)} icon={<Trash className="text-red-500" />} />
            </>
          )}
        </CardContent>
      </Card>
      ...
      <p className="text-sm text-muted-foreground mt-6">
        Je accountgegevens worden beveiligd volgens onze privacyvoorwaarden. Voor specifieke vragen kun je contact opnemen met support.
      </p>
        ...
        <DialogContent className="bg-neutral-800 text-white border-neutral-700">
          ...
          <div className="py-4 space-y-4">
            <div className="flex items-center space-x-4">
              <Switch id="dialog-delete-lists" checked={deleteLists} onCheckedChange={setDeleteLists} />
              <Label htmlFor="dialog-delete-lists" className="text-white">Verwijder lijsten + samenvattingen</Label>
            </div>
            <div className="flex items-center space-x-4">
              <Switch id="dialog-delete-forum" checked={deleteForumPosts} onCheckedChange={setDeleteForumPosts} />
              <Label htmlFor="dialog-delete-forum" className="text-white">Verwijder forumposts</Label>
            </div>
            <p className="text-white">
              Bij het verwijderen van je account worden al je persoonlijke gegevens, voortgang en lijsten permanent verwijderd.
            </p>
            <div className="flex justify-end space-x-2">
              <Button1 text="Annuleer" onClick={() => setDeleteDialogOpen(false)} />
              <Button1 text={deleteLoading ? "Verwijderen..." : "Bevestigen"} onClick={handleDeleteAccount} disabled={deleteLoading} />
            </div>
          </div>
        </DialogContent>
      </Dialog>
