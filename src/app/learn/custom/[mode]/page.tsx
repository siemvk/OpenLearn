...
const modeMapping: Record<
  string,
  { mode: LearningMode; currentMethod: string }
> = {
...
export default async function CustomLearnPage({
  ...
  // Check if this is a combined list from multiple selections
  if (listId.startsWith("combined-")) {
    // For combined lists, we'll load the data on the client side from sessionStorage
    // Pass empty array here, the client will populate it
    rawListData = [];
  } else {
    ...
    const { addToRecentLists } = await import(
      "@/utils/actions/updateRecentLists"
    );
    const { addToRecentSubjects } = await import(
      "@/utils/actions/updateRecentSubjects"
    );
    ...
    rawListData = selectedWordPairs.map((item: any) => ({
      vraag: item["1"] || "",
      antwoord: item["2"] || "",
    }));
  }

  // If no valid selected pairs and not a combined list, redirect to home start
  if (rawListData.length === 0 && !listId.startsWith("combined-")) {
    redirect("/home/start");
  }
  ...
  return (
    <CustomLearnClient
      ...
      isCombinedList={listId.startsWith("combined-")}
    />
  );
}
